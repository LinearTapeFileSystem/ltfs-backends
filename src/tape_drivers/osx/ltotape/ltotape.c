...
int ltotape_remaining_capacity (void *device, struct tc_remaining_cap *cap)
{
  ...
  status = ltotape_logsense (device, LOG_PAGE_TAPE_CAPACITY, 0x00, logdata, LOGSENSEPAGE);
  if (status < 0) {
 ...
 */
  ...
  rc = ltotape_logsense (device, LOG_PAGE_VOLUMESTATS, 0x00, logdata, LOGSENSEPAGE);
  if (rc < 0) {
...
int ltotape_get_tape_alert (void *device, uint64_t* taflags)
{
  ...
  *taflags = 0;
  ...
  rc = ltotape_logsense (device, LOG_PAGE_TAPE_ALERT, 0x00, logdata, LOGSENSEPAGE);
  if (rc < 0) {
